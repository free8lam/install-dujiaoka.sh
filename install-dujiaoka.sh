...
require_root() {
  if [[ $EUID -ne 0 ]]; then
    error "请以 root 或使用 sudo 执行。"
    exit 1
  fi
  if ! grep -qi "Ubuntu 24.04" /etc/os-release; then
    error "检测到非 Ubuntu 24.04 系统。"
    exit 1
  fi
}
...
input_config() {
  ...
  read -r -p "WordPress 网站标题（默认 WordPress）: " TITLE
  ...
  read -r -p "请输入需要开放的端口（多个端口用空格分隔，例如 80 443 10888）: " OPEN_PORTS
}
...
setup_dns_fix() {
  ...
  log "已设置 DNS 以确保 WordPress.org 可访问"
}
...
summary() {
  ...
  echo "WP Cron 已设置为系统任务，每 5 分钟运行"
  echo "已安装缓存插件：WP Super Cache"
  echo "请确保 VPS 可以访问 WordPress.org API（cURL）"
  echo "------------------------------------------------------------"
}
...
menu_existing() {
  ...
  read -r -p "输入选项数字: " choice
  case "$choice" in
    1) load_config ;;
    2) input_config; save_config ;;
    3) load_config; install_packages; detect_php; tune_php; configure_nginx; configure_ufw; obtain_ssl; summary; exit 0 ;;
    4)
      load_config
      log "执行完全重装..."
      ...
      rm -rf "${WP_PATH}"
      input_config; save_config
      ;;
    *) echo "无效选项，默认使用现有配置"; load_config ;;
  esac
}
